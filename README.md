...
### ПГЗ — прямая геодезическая задача

От точки стояния последовательно откладываются направления и расстояния. Каждая строка — **горизонтальный угол (ГУ)** в градусах, минутах, секундах и **расстояние s** в метрах.

Азимут луча: **A = A1 + ГУ**.

Для каждой вычисленной точки (№ 3, 4, …) в результате выводятся:

- широта **B** и долгота **L** ;
- **A** — прямой азимут направления;
- **Aобр** — обратный азимут в конечной точке.

### Экспорт

После решения ОГЗ и ПГЗ доступны три формата:

| Формат | Описание |
|--------|----------|
| **PDF** | Ведомость координат (таблица: №, Широта B, Долгота L) + шапка с параметрами эллипсоида и исходными данными |
| **KML** | Точки и лучи от станции для Google Earth |
| **Карта** | HTML-страница с Leaflet (OpenStreetMap), маркеры и линии |

Файлы сохраняются в каталог `exports/` во внутреннее хранилище приложения на Android.
...
В этом файле содержится ядро геодезических вычислений. Константы по умолчанию: WGS-84 (`a = 6378137 м`, `e² = 0.00669437999014`).

| Функция | Назначение |
|---------|------------|
| `dms_to_deg`, `deg_to_dms`, `format_dms` | Преобразование и форматирование градусов, минут и секунд |
| `normalize_deg_0_360`, `normalize_lon` | Нормализация углов |
| `inverse_geodetic_problem` | **ОГЗ** — итерационный метод |
| `direct_geodetic_problem` | **ПГЗ** — координаты конечной точки по азимуту и расстоянию |
...
| `solve_ogz(payload)` | `Geocalc.ogz()` / main.py | ОГЗ; проверка входных данных (валидация): a, e², кол-во итераций |
...
| `open_path(path, mime)` | Открытие файла (webbrowser / plyer) |
...
- Три секций: **ОГЗ**, **ПГЗ**, **Экспорт**.
- В свёрнутом виде три крупные кнопки занимают экран; при раскрытии секции открывается форма ввода.
- ОГЗ: блоки «Точка стояния» (B1, L1) и «Точка ориентирования» (B2, L2); поля a, e², Итерации.
- ПГЗ: строки «ГУ (° ′ ″) + расстояние s»; кнопки «+» / «−» для добавления/удаления строк.
- Отработка ошибок введённых данных — модальное окно (message box).
...
2. Формируется массив `points` (исходные и вычисленные точки) и `meta` (a, e², координаты, A1).
3. `Geocalc.exportFile(payload, kind)` → файл → Intent «Открыть» (PDF/KML/HTML).
...
Workflow `.github/workflows/build-apk.yml` — сборка через **Buildozer** (`kivy/buildozer-action`). Альтернативный путь; основной — `build_apk.py`.
